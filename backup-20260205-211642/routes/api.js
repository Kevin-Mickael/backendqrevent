...
const { validateQRCode } = require('../middleware/security');
const qrCodeService = require('../services/qrCodeService');
const { users, events, guests, qrCodes, attendance, families, familyInvitations, familyRsvp, storyEvents, games } = require('../utils/database');
...
router.post('/verify-qr/:qrCode', validateQRCode, async (req, res) => {
  try {
...
router.get('/invitations', authenticateToken, async (req, res) => {
  try {
    ...
    // Enrich with stats for each event
    const invitations = await Promise.all(
      eventsList.map(async (event) => {
        ...
        // Calculate stats
        const confirmed = guestsList.filter(g => g.rsvp_status === 'accepted').length;
        ...
        // For now, views is mocked (will be implemented with analytics later)
        const views = 0;
        
        // Determine status based on event date and is_active
        ...
        return {
          ...
          status: status,
          views: views,
          responses: confirmed + declined + pending,
          confirmed: confirmed,
          declined: declined,
          pending: pending,
...
router.get('/dashboard/summary', authenticateToken, async (req, res) => {
  try {
    // 1. Get user's events (latest first)
    const eventsList = await events.findByOrganizer(req.user.id);
    ...
    // 2. Get data for the latest active event
    const latestEvent = eventsList[0];
    summary.latestEvent = latestEvent;

    // 3. Get guests for this event
    const guestsList = await guests.findByEvent(latestEvent.id);

    // 4. Calculate stats
    summary.stats.totalGuests = guestsList.length;
    summary.stats.confirmed = guestsList.filter(g => g.rsvp_status === 'accepted').length;
    summary.stats.declined = guestsList.filter(g => g.rsvp_status === 'declined').length;
    summary.stats.pending = guestsList.filter(g => g.rsvp_status === 'pending').length;

    // 5. Generate some recent activity (mock for now based on actual data if possible, or just return empty)
    // In a real app, we would have an 'activities' table. 
    // For now, we'll return the 5 most recently updated guests as "activity"
    const recentGuests = [...guestsList]
      ...
      .map(g => ({
        ...
        action: g.rsvp_status === 'accepted' ? 'confirmed attendance' :
          g.rsvp_status === 'declined' ? 'declined invitation' : 'updated profile',
        time: g.updated_at || g.created_at
      }));

    summary.recentActivity = recentGuests;
